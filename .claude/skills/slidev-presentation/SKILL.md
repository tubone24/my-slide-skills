...
│   ├── BookHover.vue    # 著書クリックでLightbox風プレビュー（書影＋詳細情報）
├── composables/
...
| レイアウト | 用途 | props |
|-----------|------|-------|
| `cover` | タイトルページ | `background`, `subtitle`, `titleEffect`, `titleEffectSound`, `titleEffectVolume` |
...
coverレイアウトで `titleEffect: 'typewriter'` を指定すると、タイトル文字がタイプライター風に一文字ずつ打ち出される演出になる。`titleEffectSound` で効果音ファイルを指定可能。

**使用例（空白リードインスライド付き）:**

タイプライター効果は最初のスライドではタイミングが掴めないため、必ず空白のリードインスライドを前に置くこと。クリック/キー操作でタイトルスライドに遷移した時に効果音と演出が始まる。
...
**動作の流れ:**
1. スライド表示後、効果音を再生しながら2秒間でタイトル文字を一文字ずつ打ち出す（カーソル点滅付き）
2. 2秒後（ジングル突入タイミング）に残りの文字を一気に表示し、スケールアップのインパクト演出

**プロップ一覧:**

| プロップ | 型 | デフォルト | 説明 |
|---------|-----|----------|------|
| `titleEffect` | String | `''` | `'typewriter'` でタイプライター効果を有効化 |
| `titleEffectSound` | String | `''` | 効果音ファイルのパス（public/に配置） |
| `titleEffectVolume` | Number | `0.5` | 効果音の音量（0.0〜1.0） |

**注意:** 効果音はブラウザのAutoplay Policy対策により、最初のユーザー操作（クリック/キー押下）後に有効になる。プレゼンテーション中は通常問題なし。
...
<!-- 著書プレビュー（クリックでLightbox風表示） -->
<!-- profileレイアウトの著書一覧で使用。クリックすると書影＋詳細情報がLightboxで表示される -->
<BookHover image="/books/ai-agent.webp" title="AIエージェント開発運用入門">
  <strong>AIエージェント開発運用入門</strong>（SBクリエイティブ）
  <template #detail>
    <p><strong>著者:</strong> 御田 稔 / 大坪 悠 / 塚田 真規</p>
    <p><strong>出版社:</strong> SBクリエイティブ｜<strong>刊行日:</strong> 2025年10月1日</p>
    <p>AIエージェントの開発から本番運用までをハンズオン形式で解説。</p>
  </template>
</BookHover>
<!-- image: 書影画像パス（public/books/に配置） -->
<!-- title: Lightbox内に表示されるタイトル -->
<!-- default slot: トリガーとなるテキスト（<strong>で太字可） -->
<!-- #detail slot: Lightbox内の詳細情報（著者・出版社・刊行日・概要など） -->
<!-- ESCキーまたはオーバーレイクリックで閉じる -->
<!-- 注意: Vueコンポーネント内では**が効かないため<strong>タグを使用すること -->
```
